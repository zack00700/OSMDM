...
async function api(path,opts={}){
  try{
    ...
    if(res.status===401){
      // Ne pas logout sur les appels premium en background (notifications, quality, etc.)
      if(path.includes('/premium/')) return null;
      // Ne pas logout si on est sur la page login (pas encore connecté)
      if(!TOKEN || document.getElementById('login-page').style.display==='flex') return null;
      logout();return null;
    }
...
async function openDBImport(connId,connName){const panel=document.getElementById('db-import-panel');if(!panel)return;panel.style.display='block';panel.dataset.connId=connId;document.getElementById('db-import-tables').innerHTML='<p style="color:#9ca3af;font-size:12px;">Chargement des tables...</p>';document.getElementById('db-preview').innerHTML='';document.getElementById('db-import-progress').style.display='none';const res=await api(`/connections/${connId}/tables`);const container=document.getElementById('db-import-tables');if(res?.tables?.length){container.innerHTML=res.tables.map(t=>`<label style="display:flex;align-items:center;gap:6px;padding:4px 8px;border-radius:6px;cursor:pointer;font-size:12px;color:#374151;transition:background .1s;" onmouseover="this.style.background='#e8f0fb'" onmouseout="this.style.background='transparent'"><input type="checkbox" class="db-table-check" value="${esc(t)}" onchange="updateTableCount()" style="width:14px;height:14px;accent-color:#1B5EA6;"/>${esc(t)}</label>`).join('');} else {container.innerHTML='<p style="color:#9ca3af;font-size:12px;">Aucune table trouvée.</p>';}updateTableCount();}
function selectAllTables(checked){document.querySelectorAll('.db-table-check').forEach(c=>c.checked=checked);updateTableCount();}
function updateTableCount(){const count=document.querySelectorAll('.db-table-check:checked').length;const el=document.getElementById('db-tables-count');if(el)el.textContent=`${count} table(s) sélectionnée(s)`;}
function getSelectedTables(){return[...document.querySelectorAll('.db-table-check:checked')].map(c=>c.value);}
async function previewTable(){const connId=document.getElementById('db-import-panel').dataset.connId;const tables=getSelectedTables();const sql=document.getElementById('db-import-sql').value.trim();const table=tables[0]||'';if(!table&&!sql){toast('Sélectionnez au moins une table ou écrivez du SQL','error');return;}const res=await api(`/connections/${connId}/preview`,{method:'POST',body:JSON.stringify({table,sql:sql||null})});if(!res?.rows?.length){document.getElementById('db-preview').innerHTML='<p style="color:#9ca3af;font-size:12px;">Aucun résultat.</p>';return;}const cols=res.columns||Object.keys(res.rows[0]);document.getElementById('db-preview').innerHTML=`<p style="font-size:11px;color:#6b7280;margin-bottom:6px;">Aperçu de <b>${esc(table||'SQL')}</b> (${res.rows.length} lignes)</p><div style="overflow-x:auto;max-height:200px;"><table style="width:100%;border-collapse:collapse;font-size:11px;"><thead><tr>${cols.map(c=>`<th class="table-th">${esc(c)}</th>`).join('')}</tr></thead><tbody>${res.rows.map(r=>`<tr>${cols.map(c=>`<td class="table-td">${esc(String(r[c]??''))}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;}
async function importFromDB(){const connId=document.getElementById('db-import-panel').dataset.connId;const sql=document.getElementById('db-import-sql').value.trim();const baseLabel=document.getElementById('db-import-label').value.trim();const progressEl=document.getElementById('db-import-progress');const btn=document.getElementById('btn-import-db');if(sql){btn.disabled=true;btn.textContent='⏳ Import...';toast('Import SQL en cours…','info');const source=baseLabel||'SQL custom';const res=await api(`/connections/${connId}/import`,{method:'POST',body:JSON.stringify({table:'',query:sql,source_label:source})});btn.disabled=false;btn.textContent='📥 Importer les tables sélectionnées';if(res){toast(`✅ ${res.imported} lignes importées`);document.getElementById('db-import-panel').style.display='none';loadDashboard();}return;}const tables=getSelectedTables();if(!tables.length){toast('Sélectionnez au moins une table','error');return;}btn.disabled=true;progressEl.style.display='block';let totalImported=0,totalErrors=0;for(let i=0;i<tables.length;i++){const t=tables[i];const source=baseLabel?`${baseLabel}.${t}`:t;progressEl.innerHTML=`⏳ Import table <b>${esc(t)}</b> (${i+1}/${tables.length})...`;const res=await api(`/connections/${connId}/import`,{method:'POST',body:JSON.stringify({table:t,source_label:source})});if(res){totalImported+=res.imported||0;totalErrors+=res.errors||0;}}btn.disabled=false;btn.textContent='📥 Importer les tables sélectionnées';progressEl.innerHTML=`✅ <b>${tables.length} table(s)</b> importée(s) — ${totalImported} lignes au total, ${totalErrors} erreur(s)`;progressEl.style.background=totalErrors?'#fef3c7':'#d1fae5';progressEl.style.color=totalErrors?'#92400e':'#065f46';toast(`✅ ${totalImported} lignes importées depuis ${tables.length} table(s)`);loadDashboard();}
