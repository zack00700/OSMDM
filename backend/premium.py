...
# ── AUTH : vérifier le token dans before_request du blueprint ─────
# Les routes publiques (aucune pour l'instant) seraient listées ici
_PUBLIC_ROUTES = set()

@premium_bp.before_request
def premium_check_auth():
    """Vérifie le token JWT avant chaque requête premium"""
    ...
    try:
        import jwt as _jwt
        g.current_user = _jwt.decode(token, app_module.SECRET_KEY, algorithms=['HS256'])
    except Exception:
